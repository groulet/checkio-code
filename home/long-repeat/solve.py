...
def long_repeat(line: str) -> int:
    ...
    """
    # create a set with unique value in line
    col = set(line)
    
    # search for i*unique value in line, starting with i=len(line)
    # stop if one is found
    i = len(line)
    while i >= 1:
        for l in col:
            if i*l in line: # check if the string i _times_ l is present in line
                return i
        i = i-1
