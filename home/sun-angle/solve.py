...
def sun_angle(time):
    SUNRISE = 6*60+0 # in minutes = 06:00
    SUNSET = 18*60+0 # in minutes = 18:00

    #Get the "minutes of the day"
    minutes_of_day = sum(x*int(a) for x,a in zip([60,1],time.split(":")))
    if SUNRISE <= minutes_of_day <= SUNSET:
        return 180*(minutes_of_day-SUNRISE)/(SUNSET-SUNRISE)
    else:
